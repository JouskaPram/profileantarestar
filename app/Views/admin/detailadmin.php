...
<div class="hero min-h-screen bg-base-200">
      ...
      >
        <img
          src="/img/<?= $card["gambar"]; ?>"
          class="max-w-md rounded-lg shadow-2xl h-screen"
        />
        <div class="mx-10">
        ...
        <a href="/admin/edit/<?= $card["slug"]; ?>" class="btn btn-sm btn-primary rounded-full">Edit</a>
        <form action="/admin/<?= $card["id"]; ?>" method="post" class="inline">
        <?= csrf_field(); ?>
        <input type="hidden" name="_method" value="DELETE">
      <button type="submit" class="btn btn-sm btn-primary bg-sky-400  rounded-full" onclick="return confirm('apakah yakin ingin menghapus??')">hapus</button></form>
      <!-- <a href="/admin/delete/<?= $card['id']; ?>" class="btn-md btn-secondary rounded-full">Delete</a> -->
      <a href="/admin" class="btn btn-sm btn-info rounded-full">kembali</a>
        </div>
      </div>
    </div>
<?= $this->endSection(); ?>